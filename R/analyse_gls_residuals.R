...
#' This function evaluates residuals from 'gls' model objects generated by the
#' NLME package. It returns 
#' a normality plot, residuals vs. fitted, and a lag-plot with a trend line 
#' (gls(residual ~ residual -1)) to evaluate autocorrelation. It also 
...
#' 
#' @usage
#' analyse_gls_residuals(gls_object)
#' 
#' @importFrom magrittr %>% 
...
analyse_gls_residuals <- function(gls_object) {
  ...
  resi <- gls_object$residuals
  resDf <- as.data.frame(resi)
  fit <- gls_object$fitted
  ...
  autocorrelation <- stats::acf(gls_object$residuals, 
                                type = "correlation",
